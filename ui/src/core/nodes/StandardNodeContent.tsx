...
function useIsTruncated(ref: React.RefObject<HTMLElement | null>) {
  ...
  useEffect(() => {
    ...
    if (!el) return
    const check = () => setTruncated(el.scrollHeight > el.clientHeight || el.scrollWidth > el.clientWidth)
    check()
...
function TitleWithTooltip({ label, className }: { label: string; className: string }) {
  ...
  const title = <p ref={ref} className={className}>{label}</p>
...
export function StandardNodeContent({ data, compact = false }: StandardNodeContentProps) {
  ...
  return (
    <div className={clsx('flex h-full flex-col justify-center gap-1', compact ? 'px-3 py-2' : 'px-3.5 py-2.5')}>
      {eyebrow ? (
        <span
